...
mod tests {
    ...
    async fn can_add_remove_pipeline() -> anyhow::Result<()> {
        internal_init_logger(None, None).ok();
        let mut pipeline = MultiFieldPipeline::new("test_p_cap_57", json!({}).into());
    ...
    async fn can_add_remove_pipelines() -> anyhow::Result<()> {
        internal_init_logger(None, None).ok();
        let mut pipeline1 = MultiFieldPipeline::new("test_r_p_carps_1", json!({}).into());
        let mut pipeline2 = MultiFieldPipeline::new("test_r_p_carps_2", json!({}).into());
    ...
    async fn can_upsert_documents_and_add_pipeline() -> anyhow::Result<()> {
        internal_init_logger(None, None).ok();
        let collection_name = "test_r_c_cudaap_33";
        let mut collection = Collection::new(collection_name, None);
        let documents = generate_dummy_documents(2);
        collection.upsert_documents(documents.clone(), None).await?;
    ...
    #[sqlx::test]
    async fn can_search() -> anyhow::Result<()> {
        internal_init_logger(None, None).ok();
        let collection_name = "test_r_c_cs_34";
        let mut collection = Collection::new(collection_name, None);
        // let documents = generate_dummy_documents(10);
        // collection.upsert_documents(documents.clone(), None).await?;
        let pipeline_name = "test_r_p_cs_6";
        let mut pipeline = MultiFieldPipeline::new(
            pipeline_name,
            json!({
                "title": {
