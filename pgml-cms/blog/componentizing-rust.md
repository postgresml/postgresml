...
We then add Sailfish templates to this app, in the cargo.toml file add `sailfish = "0.8.0" # 0.8.1 has breaking changes` under dependencies. 
...
Now lets make a second component called page.  In page/mod.rs place the following code.
...
```
in page/template.html put a div that contains `<div>Hello from page</div>`.
...
If we have done this all correctly our file structure should be
- hello-component
  - src 
    - components
      - mod.rs
...
Now we would like to render our page inside our layout when the user visits our site, to do that, in main.rs we return the layout and set the page parameter to our page component.  To do this add the following code to main.rs: 
```rust
#[get("/")]
fn index() -> ResponseOk {
...
```
Now if we visit the "/" endpoint of our site we will see 
...
Using templates to make components is the goto technique for web developers creating server side rendered apps. If this is not you, please visit [sailfish](https://rust-sailfish.github.io/sailfish/) for more info.  
...
Now we need to make are Copy component controller.  In static/js make a file names Copy and add 
