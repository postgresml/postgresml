...
pub fn load_dataset(name: &str, subset: Option<String>, limit: Option<usize>, kwargs: &serde_json::Value) -> usize {
    ...
    let dataset = Python::with_gil(|py| -> String {
        ...
        load_dataset.call1(
            py, PyTuple::new(py, &[name.into_py(py), subset.into_py(py), limit.into_py(py), kwargs.into_py(py)])
        )
            .unwrap()
            .extract(py)
            .unwrap()
    ...
    let column_names = columns.iter().map(|(key, _values)| key.clone() ).collect::<Vec<String>>().join(", ");
    let column_types = columns.iter().map(|(key, values)| {
    ...
    let placeholders = columns.iter().enumerate().map(|(i, _)| {
        let placeholder = i + 1;
        format!("${placeholder}")
    }).collect::<Vec<String>>().join(", ");
    ...
    for i in 0..num_rows {
        ...
        for (_column, values) in columns {
            ...
            if value.is_boolean() {
                row.push((PgBuiltInOids::BOOLOID.oid(), value.as_bool().unwrap().into_datum()));
            } else if value.is_i64() {
                row.push((PgBuiltInOids::INT8OID.oid(), value.as_i64().unwrap().into_datum()));
            } else if value.is_f64() {
                row.push((PgBuiltInOids::FLOAT8OID.oid(), value.as_f64().unwrap().into_datum()));
            } else if value.is_string() {
                row.push((PgBuiltInOids::TEXTOID.oid(), value.as_str().unwrap().into_datum()));
            } else if value.is_object() {
                row.push((PgBuiltInOids::JSONBOID.oid(), JsonB(value.clone()).into_datum()));
            } else {
